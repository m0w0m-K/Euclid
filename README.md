...
스냅은 카메라 이동, 길 이동, 길 위치, 자유 이동 구간 등 현재 지원되는 위치 이펙트에서 사용할 수 있습니다.

장식 이동은 여러 장식을 태그로 동시에 제어하고 기준 좌표, 이전 위치, 시차 등의 상태를 함께 고려해야 하므로 현재 위치 마크 및 스냅 지원 대상에 포함하지 않습니다.
...
현재 위치 마크를 지원하는 이펙트는 카메라 이동, 길 이동, 길 위치, 자유 이동 구간입니다.
...
`모든 이펙트 마크 표시`는 색상 설정보다 위쪽에 배치되어 있습니다.

이펙트별 색상 설정은 접고 펼칠 수 있으며, 필요한 이펙트의 설정만 펼쳐서 수정할 수 있습니다.
